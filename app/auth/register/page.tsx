'use client'

import { useState, FormEvent } from 'react'
import Link from 'next/link'
import { useRouter } from 'next/navigation'
import { useAuth } from '@/contexts/AuthContext'
import { useLang } from '@/contexts/LanguageContext'
import toast from 'react-hot-toast'

export default function RegisterPage() {
  const { t } = useLang()
  const { signUp, signInWithGoogle } = useAuth()
  const router = useRouter()
  const [name, setName]         = useState('')
  const [email, setEmail]       = useState('')
  const [password, setPassword] = useState('')
  const [loading, setLoading]   = useState(false)

  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault()
    if (password.length < 6) {
      toast.error('Password must be at least 6 characters')
      return
    }
    setLoading(true)
    try {
      await signUp(email, password, name)
      toast.success('Account created! Welcome')
      router.push('/account')
    } catch {
      toast.error('Could not create account. Email may already be in use.')
    } finally {
      setLoading(false)
    }
  }

  const handleGoogle = async () => {
    try {
      await signInWithGoogle()
      router.push('/account')
    } catch {
      toast.error('Google sign-in failed')
    }
  }

  return (
    <main className="min-h-screen bg-cream flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-md">

        {/* Brand */}
        <div className="text-center mb-10">
          <Link href="/" className="font-playfair text-2xl font-bold text-brown">
            Yasmina&apos;s Bites
          </Link>
          <p className="font-dancing text-gold text-lg mt-1">{t('auth_join')}</p>
        </div>

        <div className="bg-white rounded-3xl shadow-sm p-8">
          <h1 className="font-playfair text-2xl font-bold text-brown mb-6">{t('auth_create')}</h1>

          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              <label className="block text-sm font-medium text-brown mb-1.5">{t('auth_full_name')}</label>
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                required
                className="w-full px-4 py-3 rounded-xl border border-gold-pale bg-cream text-brown text-sm focus:outline-none focus:ring-2 focus:ring-gold/30"
                placeholder="Yasmina"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-brown mb-1.5">{t('auth_email')}</label>
              <input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                required
                className="w-full px-4 py-3 rounded-xl border border-gold-pale bg-cream text-brown text-sm focus:outline-none focus:ring-2 focus:ring-gold/30"
                placeholder="you@example.com"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-brown mb-1.5">{t('auth_password')}</label>
              <input
                type="password"
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                required
                className="w-full px-4 py-3 rounded-xl border border-gold-pale bg-cream text-brown text-sm focus:outline-none focus:ring-2 focus:ring-gold/30"
                placeholder={t('auth_min_pw')}
              />
            </div>

            <button
              type="submit"
              disabled={loading}
              className="bg-gold text-cream py-3 rounded-full font-semibold text-sm hover:bg-gold-light transition-colors mt-2 disabled:opacity-50"
            >
              {loading ? t('auth_creating') : t('auth_create')}
            </button>
          </form>

          <div className="relative my-6 flex items-center gap-4">
            <div className="flex-1 h-px bg-gold-pale" />
            <span className="text-xs text-brown-light">{t('auth_or')}</span>
            <div className="flex-1 h-px bg-gold-pale" />
          </div>

          <button
            onClick={handleGoogle}
            className="w-full border border-gold-pale text-brown py-3 rounded-full text-sm font-semibold hover:bg-gold-pale transition-colors flex items-center justify-center gap-2"
          >
            <svg viewBox="0 0 24 24" className="w-4 h-4">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            {t('auth_google')}
          </button>

          <p className="text-center text-sm text-brown-light mt-6">
            {t('auth_has_account')}{' '}
            <Link href="/auth/login" className="text-gold font-semibold hover:underline">
              {t('auth_sign_in_link')}
            </Link>
          </p>
        </div>
      </div>
    </main>
  )
}
